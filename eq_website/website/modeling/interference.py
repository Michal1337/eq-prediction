...
def get_data(
    ...
    count = get_earthquake_count(
        make_params(start_time, end_time, min_lat, max_lat, min_lon, max_lon), data_source
    )
    if count == 0:
    ...
    while count < 65:
        ...
        if data_source == 'local':
            count = get_earthquake_count(
                make_params(start_time, end_time, Y-5, Y+5, X-5, X+5), data_source
            )
        else:
            count = get_earthquake_count(
                make_params_circle(start_time, end_time, Y, X, radius + 10), data_source
            )
        cnt += 1
    ...
    while cnt < 20:
        ...
        for i in range(len(dates) - 1):
            ...
            if data_source == 'usgs':
                ...
                df_small["time"] = pd.to_datetime(df_small["time"], format="mixed")
                df_small['time'] = df_small["time"].dt.tz_localize(None)
...
def prepare_data(df: pd.DataFrame, geo_split: int) -> pd.DataFrame:
    ...
    """
    df_tp = pd.read_csv("../assets/data/plates.csv")
    df_tp.drop_duplicates(inplace=True)

    df = add_region_info(df, df_tp, geo_split)
    df = add_tectonic_info(df, df_tp)

    mapping1 = pd.read_csv("../assets/data/magtype2id.csv")
    mapping2 = pd.read_csv("../assets/data/plate2id.csv")
    mapping3 = pd.read_csv("../assets/data/plate_region2id.csv")
    ...
    df['magType'] = df['magType'].fillna(18)
    return df


def make_prediction(X: float, Y: float, data_source: str, path: str) -> Union[float, str]:
    """
    ...
    """
    regions = np.load("../assets/data/regions.npy", allow_pickle=True)
    ...
    scalers = pickle.load(open("../assets/data/scalers_for_npys.pkl", "rb"))
    ...
    try:
        return model.predict([x_ts.astype(np.float32), x_region.astype(np.float32)])[0][0]
    except ValueError:
        return model.predict(x_ts.astype(np.float32))[0][0]
